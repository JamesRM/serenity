...
constexpr uint64_t* descriptor_to_pointer(FlatPtr descriptor)
{
...
private:
    void zero_page(uint64_t* page)
    {
        // Memset all page table memory to zero
        for (uint64_t* p = page; p < page + (PAGE_TABLE_SIZE / sizeof(uint64_t)); p++) {
            *p = 0;
        }
    }
...
static void insert_identity_entries_for_physical_memory_range(PageBumpAllocator& allocator, uint64_t* page_table, FlatPtr start, FlatPtr end, uint64_t flags)
{
    ...
    for (FlatPtr addr = start; addr < end; addr += GRANULE_SIZE) {
        ...
        uint64_t level0_idx = (addr >> 39) & 0x1FF;
        uint64_t level1_idx = (addr >> 30) & 0x1FF;
        uint64_t level2_idx = (addr >> 21) & 0x1FF;
        uint64_t level3_idx = (addr >> 12) & 0x1FF;

        uint64_t* level1_table = page_table;

        if (level1_table[level0_idx] == 0) {
            level1_table[level0_idx] = (FlatPtr)allocator.TakePage();
            level1_table[level0_idx] |= TABLE_DESCRIPTOR;
        }

        uint64_t* level2_table = descriptor_to_pointer(level1_table[level0_idx]);

        if (level2_table[level1_idx] == 0) {
            level2_table[level1_idx] = (FlatPtr)allocator.TakePage();
            level2_table[level1_idx] |= TABLE_DESCRIPTOR;
        }

        uint64_t* level3_table = descriptor_to_pointer(level2_table[level1_idx]);

        if (level3_table[level2_idx] == 0) {
            level3_table[level2_idx] = (FlatPtr)allocator.TakePage();
            level3_table[level2_idx] |= TABLE_DESCRIPTOR;
        }

        uint64_t* level4_table = descriptor_to_pointer(level3_table[level2_idx]);
        uint64_t* l4_entry = &level4_table[level3_idx];
        *l4_entry = addr;
        *l4_entry |= flags;
    }
}
...
static void build_identity_map(PageBumpAllocator& allocator)
{
    ...
    uint64_t normal_memory_flags = ACCESS_FLAG | PAGE_DESCRIPTOR | INNER_SHAREABLE | NORMAL_MEMORY;
    uint64_t device_memory_flags = ACCESS_FLAG | PAGE_DESCRIPTOR | OUTER_SHAREABLE | DEVICE_MEMORY;
    ...
    insert_identity_entries_for_physical_memory_range(allocator, level1_table, MMIO::the().PeripheralBaseAddress(), MMIO::the().PeripheralEndAddress(), device_memory_flags);
}
